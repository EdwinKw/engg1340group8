...
void alertcheck(list mainlist[]) {
	for (int i = 0; i < itemcount("dalist.txt"); i++) {
		if (mainlist[i].number == 0 or mainlist[i].stock == 0) {
...
void afteradd(int stock, int item, list mainlist[]) {
	for (int i = 0; i < itemcount("dalist.txt") - 1; i++) {
		mainlist[item].rec[i] = mainlist[item].rec[i + 1];
	}
	mainlist[item].rec[itemcount("dalist.txt") - 1] = stock;
}
...
void adding(list mainlist[], int location, int add) {
	...
	afteradd(mainlist[location].number, location, mainlist);
}
...
void update(list mainlist[], string filename, int n) {
	...
	for (int i = 0; i < n; i++) {
		...
		if (mainlist[i].number <= 0)
			...
			mainlist[i].stock = 1;
		if (mainlist[i].stock = 0 or mainlist[i].number <= 0)
